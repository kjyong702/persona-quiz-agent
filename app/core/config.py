...
class Settings(BaseSettings):
    ...
    # 요청 하나의 상한. **지금까지 이 값이 없었다.**
    #
    # 호출 단위 타임아웃(llm_timeout_seconds)만 있으면 상한이 안 잡힌다.
    # 재시도가 4회고 백오프가 붙으므로 호출 하나가 최악 83.5초이고,
    # 판정 한 건은 임베딩과 LLM을 둘 다 부르므로 167초까지 간다.
    #
    # **앱이 앞단보다 먼저 끊는다.** 프록시가 먼저 끊으면 클라이언트는 504를 받는데
    # 앱은 계속 LLM을 부른다. 아무도 안 기다리는 응답에 돈을 쓰는 셈이고,
    # 그 사실이 어디에도 안 남는다. 앱이 끊으면 작업이 함께 취소되고
    # 도메인 에러 코드로 응답할 수 있다.
    #
    # 앞단을 둘 때는 이 값보다 **길게** 잡아야 한다. 짧으면 앞단이 먼저 이긴다
