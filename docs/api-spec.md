...
### GET /metrics
나가는 외부 호출의 누적 카운터. 부하 실험에서 대응 전후를 비교할 때 쓴다.

```json
{
  "counters": {
    "embedding.attempt": 120,
    "embedding.success": 120,
    "llm.attempt": 34,
    "llm.rate_limited": 6,
    "llm.retry_after_honored": 4,
    "llm.giveup": 0
  },
  "waits": { "llm.throttle_wait_seconds": { "count": 22, "total": 4.81, "max": 0.62 } }
}
```

- 클라이언트가 보는 성공률만으로는 부족하다. **429가 몇 번 났는지는 재시도가 흡수해버려 밖에서 안 보인다.** 대응이 실제로 일한 양을 재려면 서버 안쪽 카운터가 필요하다
- 프로세스 메모리에만 있고 재시작하면 사라진다. 관측 도구를 붙이는 것은 이 프로젝트 범위 밖이고, 여기서는 부하 실험의 계측 지점으로만 쓴다
